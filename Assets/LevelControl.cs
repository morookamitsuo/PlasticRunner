...
public class LevelData{
		...
		public LevelData(){ //コンストラクタ
				...
				this.floor_count.max = 10; // 足場ブロックの数の最大値を初期化
...
public class LevelControl : MonoBehaviour{
		...
		public CreationInfo previos_block;            // 前回、どういうブロックを作ったか
		public CreationInfo current_block;            // 今回、どういうブロックを作ったか
		public CreationInfo next_block;               // 次回、どういうブロックを作るべきか

		public int block_count = 0;                   // 作成したブロックの個数
		...
		private void update_level(ref CreationInfo current, CreationInfo previous, float passege_time){
				...
				//Mathf.Repeat(value, max)  valueの値がなんであれ0〜maxの値が返される
		...
		private void update_level(ref CreationInfo current,CreationInfo previos){
				...
				case Block.TYPE.FLOOR:                        // 今回のブロックが床の場合
						...
						current.height = previos.height;      // 高さを前回と同じにする
						break;
		...
		*/
		...
		public void update(float passage_time){
				...
				if(this.current_block.current_count >= this.current_block.max_count){
						...
						this.clear_next_block (ref this.next_block);
						// 次に作るべきブロックを設定
						// this.update_level (ref this.next_block, this.current_block); // テキストデータを反映させる前のupdate_level
